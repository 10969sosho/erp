...
const shortcuts = [
  { href:'/workspace/purchase-requests', label:'Purchase request', icon:'PR', color:'#8b5cf6' }, { href:'/workspace/sales-orders', label:'Sales order', icon:'SO', color:'#ec4899' },
  { href:'/workspace/stock-adjustments', label:'Stock adjustment', icon:'AD', color:'#f97316' }, { href:'/workspace/journals', label:'General journal', icon:'JV', color:'#10b981' },
];
...
export default function Dashboard() {
  ...
  return <div className="page-content dashboard-page">
    ...
    <section className="metric-grid">
      <article className="metric-card"><div><span>Nilai inventory</span><i style={{background:'linear-gradient(135deg,#3b82f6,#06b6d4)',color:'#fff',border:0}}>IV</i></div><strong>{money(stockValue)}</strong><p>{inventory.length} gudang tercatat</p></article>
      <article className="metric-card card-amber"><div><span>Piutang terbuka</span><i style={{background:'linear-gradient(135deg,#f97316,#f59e0b)',color:'#fff',border:0}}>AR</i></div><strong>{money(aging.ar)}</strong><p>Seluruh bucket aging</p></article>
      <article className="metric-card card-coral"><div><span>Hutang terbuka</span><i style={{background:'linear-gradient(135deg,#ec4899,#8b5cf6)',color:'#fff',border:0}}>AP</i></div><strong>{money(aging.ap)}</strong><p>Invoice belum lunas</p></article>
      <article className="metric-card card-blue"><div><span>Perlu perhatian</span><i style={{background:'linear-gradient(135deg,#6366f1,#8b5cf6)',color:'#fff',border:0}}>NT</i></div><strong>{notifications.filter((item) => item.status === 'unread').length}</strong><p>Notifikasi belum dibaca</p></article>
    </section>
    <section className="dashboard-grid">
      ...
      <article className="panel shortcut-panel"><div className="panel-heading"><div><p className="eyebrow">QUICK ACTIONS</p><h2>Mulai pekerjaan</h2></div></div><div className="shortcut-grid">{shortcuts.map((item) => <Link href={item.href} key={item.href}><span style={{background:`linear-gradient(135deg,${item.color},${item.color}cc)`}}>{item.icon}</span><b>{item.label}</b><i>→</i></Link>)}</div></article>
